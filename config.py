...
MIN_POSITION_SIZE = 2.0  # Минимальный размер позиции в USD (временно снижено для тестирования)

# Настройки для короткосрочной торговли
SHORT_TERM_TRADING = {
    'enable': True,  # Включить короткосрочную торговлю
    'max_holding_hours': 24,  # Максимальное время удержания позиции (часы)
    'quick_profit_target': 0.02,  # Цель быстрой прибыли (2%)
    'stop_loss_percent': 0.015,  # Стоп-лосс для короткосрочных позиций (1.5%)
    'min_profit_threshold': 0.005,  # Минимальный порог прибыли для выхода (0.5%)
    'exit_confidence_threshold': 0.7,  # Минимальная уверенность ML для выхода
    'scalping_mode': True,  # Режим скальпинга для быстрых сделок
    'preferred_timeframes': ['5m', '15m', '1h'],  # Предпочтительные таймфреймы
}
...
ML_CONFIG = {
    ...
    'prediction_horizon': 6,  # Горизонт прогнозирования (свечей) - увеличено с 1 до 6 для прогноза на 6 часов
